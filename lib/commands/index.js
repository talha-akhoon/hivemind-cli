...
const {runAgentLoop} = require("./job");
    ...
    const answers = await inquirer.prompt([
        ...
        },
        {
            ...
            validate: (input) => {
                ...
                if (value > 5) return 'ROI seems to high (max 5)';
                return true;
            },
...
${chalk.gray('Capabilities:')} ${chalk.cyan(config.capabilities.join(', '))}
${chalk.gray('Min ROI:')} ${chalk.cyan(config.minROI + 'x')}

${chalk.green('🎯 Agent is now scanning for profitable training opportunities...')}
${chalk.green('📊 Workflows managed by Inngest for reliability and monitoring')}
...
Your agent needs HBAR to participate in training jobs.
Recommended minimum: 100 HBAR
`));
        }

        // Start Inngest server and workflows
        const { inngest } = require('../inngest/client');

        // Start the Inngest server
        const inngestServer = require('../inngest/server');
        const port = 3003;

        const server = inngestServer.listen(port, () => {
            console.log(chalk.blue(`🔗 Inngest server running on port ${port}`));
            console.log(chalk.gray(`📊 Dashboard: http://localhost:${port}/api/inngest`));
        });

        // Trigger initial marketplace scan
        await inngest.send({
            name: 'marketplace/scan-trigger',
            data: { agentId: config.machineId }
        });

        // Handle graceful shutdown
        process.on('SIGINT', async () => {
            console.log(chalk.yellow('\n🛑 Stopping HiveMind Agent...'));
            server.close();
            process.exit(0);
        });
        ...
        if (error.code === 'ENOENT') {
            ...
            console.log(chalk.yellow('Run "hivemind init" first to set up your agent.'));
        } else if (error.message && error.message.includes('fetch failed')) {
            console.error(chalk.red('\n❌ Network error: Unable to reach Inngest API endpoint.'));
            console.log(chalk.yellow('Check your internet connection or Inngest configuration.'));
        } else {
            console.error(chalk.red('\n❌ Error:', error.message));
